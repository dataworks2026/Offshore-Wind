...
import yaml
from main.inference import auto_find_best_pt, validate, predict_coastal, predict_detections
...
def eval_predict_per_class_pr(
    ...
    for img_path in image_paths:
        ...
        if len(preds) > 0:
            with_det += 1
            if not saved_with:
                ann_rgb, summary = predict_coastal(frame, model_path=weights, imgsz=640, conf=0.5, iou=0.7)
...
def parse_args():
    ...
    ap.add_argument(
        "--data-yaml",
        type=str,
        default=r"data\datasets\Coastal_Damage_Detection.v1_aug_rszd.yolov8\data.yaml",
...
def main():
    ...
    weights = r"data\models\coastal_damage_detector_cocobase.pt"
